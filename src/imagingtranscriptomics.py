...
from imaging_transcriptomics.transcriptomics import ImagingTranscriptomics
    ...
    *Daniel Martins, Alessio Giacomel, Steven CR Williams, Federico E Turkheimer, Ottavia Dipasquale, Mattia Veronese, 
    ...
    parser.add_argument("-i", "--input",
                        ...
                        help=("Input imaging file in NIfTI format (.nii, .nii.gz).\n"
                              ...
                              "resolution of the MNI152 template provided with FSL."),
                        required=True)
...
def main():
    ...
    data_to_analyse = extract_average(read_scan(inputs.input))
    initial_dict = {
        "variance": inputs.variance,
        "n_components": inputs.ncomp
    }
    analysis = ImagingTranscriptomics(data_to_analyse, initial_dict)
    analysis.run()
