...
class PDF(FPDF):
    ...
    def analysis_info(self, filename, date, filepath):
        ...
        the scan of the input, date of the analysis and the original path of
        the scan.
    ...
    def reproducibility_line(self, cli_commands, version):
        """Create a string with the command used from the command line to run
        the analysis.
...
def make_folder(path, folder_name: str):
    ...
    The function checks if in the directory there are already folders with
    the same name. If this is the case the new folder will have a trialing
    underscore with a progressive number (e.g., _2)
...
def make_plots(path, limit_x, data_y):
    """Generate the plots for the explained variance by each component, one
    with the cumulative sum and one with the
    ...
    plt.vlines(limit_x, varexp[0] - 10, varexp[limit_x - 1], colors="lightgrey",
               linestyles="dashed")
...
def create_pdf(filepath, save_dir):
    ...
    Creates a report for the imaging transcriptomics analysis with some plots
    and details of what was run by the user.

    :param filepath: path of the image used for analysis.
    :param save_dir: path where to save the report (same as where the plots
        are located).
    """
...
def create_csv(analysis_results, n_comp, save_dir):
    ...
    The function creates a different csv file for each of the components used
    for PLS regression (if 2 components are
    used the files 'PLS1.csv' and 'PLS2.csv' will be created.

    :param analysis_results: GeneResults data structure with the results of
        bootstrapping.
    ...
    for i in range(n_comp):
        data = np.vstack((np.array(
            analysis_results.boot_results.pls_genes[i].reshape(1, 15633)),
                          np.array(analysis_results.boot_results.z_scores[i]),
                          np.array(analysis_results.boot_results.pval[i]),
                          np.array(
                              analysis_results.boot_results.pval_corrected[i])
                          )).T
