...
def bootstrap_pls(x, y, y_perm, dim, iterations=1_000):
    """Return a coefficient of determination (R^2) and a p value from the
    bootstrapping on the PLS regression.

    The function calculates for each of the N ``iterations`` the value of the
    coefficient of determination  on the permuted data and determines how many
    times this is at least equal to the coefficient of determination with the
    original data.

    :param x: X data for the PLS regression (in our case the zscore of the gene
        expression data)
    :param y: Y data for the PLS regression (in our case the zscore of the
        average of the ROIs)
    :param y_perm: matrix with the permuted Y data. This should have a number
        of rows equal to the original data and number of columns equal to the
        number of iterations.
    :param int dim: number of PLS components to use for the analysis.
    :param int iterations: number of bootstrap iterations.

    :return R_boot: array of dimension ``dim`` with the coefficient of
        determination for each PLS component.
    :return p_boot: array of dimension ``dim`` with the p value for each PLS
        component.
    """
    R_boot = np.zeros(dim)
    p_boot = np.zeros(dim)
    for component in range(1, dim + 1):
        ...
        p_boot[component - 1] = float(
            len(R_sq[numpy.nonzero(R_sq >= R_squared)])) / iterations
    logger.debug("Computed components p value(s) and coefficient(s) of "
                 "determination: \n R: %s \n p: %s", R_boot, p_boot)
    return R_boot, p_boot


def bootstrap_genes(x_data, y, n_components, y_norm, genes, n_iterations=1000):
    """Run a bootstrap permuting the genes as well as the data, to assess
    reliability of the results.

    :param x_data: data to regress, provided as z scores (e.g., gene expression
        data).
    :param y: data to regress against, provided as z scores (e.g. average
        values from ROIs).
    :param n_components: number of components to use for the PLS regression.
    :param y_norm: original y data (before z scoring).
    :param genes: labels of the genes regressed against (e.g., the labels of
        the y data rows).
    :param n_iterations: number of iterations for the bootstrapping and
        permutations.
    :return gene_results: GeneResults data structure with all the results of
        the bootstrapping and the original results as well.
    """
    n_genes = 15_633
    gene_index = np.array(list(range(1, n_genes + 1)))
    results = pls_regression(x_data, y, n_components=n_components, n_boot=0,
                             n_perm=0)
    r1 = correlate(results.get("x_scores").reshape(41, n_components),
                   y_norm.reshape(41, 1))
    logger.debug("Correlation between original data and regression scores: %s",
                 r1)
    weights = results.get("x_weights")
    gene_results = GeneResults(n_components, dim1=weights.shape[0],
    ...
    for idx in range(1, n_components + 1):
        ...
        resval = np.sort(weights[:, idx - 1], kind='mergesort')[::-1]
        gene_results.original_results.set_result_values(idx, resval, x,
                                                        genes[x], gene_index[x])
    ...
    for iteration in tqdm(range(n_iterations), desc="Bootstrapping gene list",
        ...
        for component in range(1, n_components + 1):
            ...
            __new_weights = __temp[
                gene_results.original_results.index[component - 1]]
            __t_genes = np.array(
            ...
            component - 1, iteration] = __new_weights
    return gene_results
