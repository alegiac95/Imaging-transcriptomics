...
def test_check_extension_error():
    ...
    @CheckExtension
    def function(path):  # pragma: no cover
        """Dummy function to test the decorator."""
        return path
    ...
    assert (
        str(ex.value)
        == f"The provided file has an invalid format. Please use files in "
           f"the .nii, .nii.gz format. The error was caused by the file"
           f" {test_path.absolute()}/wrong_format.txt."
    )
...
def test_check_shape_error():
    ...
    @CheckShape
    def function(in_matrix):  # pragma: no cover
        """Dummy function to test the decorator."""
        return in_matrix.shape
...
def test_check_variance_error():
    ...
    @CheckVariance
    def function(var):  # pragma: no cover
        """Dummy function to test the decorator."""
        return var + 1
