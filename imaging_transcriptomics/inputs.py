...
from .errors import (check_correct_size,
                     check_path_exists,
                     check_extensions,
                     check_var_in_range)
...
def extract_average(imaging_matrix):
    ...
    for i in range(1, n_regions + 1):
        data[i] = np.mean(imaging_matrix[np.where(atlas_data == i)])
...
def load_gene_labels():
    ...
    """
    genes_labels_path = Path(__file__).resolve().parent / "data" / "genes_label.txt"
