...
``corr``
   Writes ``corr_genes.tsv`` plus one enrichment output family:
   ``ensemble_corr_results.tsv``, ``gsea_corr_results.tsv``, or
...
``gedar``
   ...
   ``matched_genes.txt``, and ``missing_genes.txt``. When requested, it also
   writes ``gsea_gedar_results.tsv`` or ``ora_gedar_up.tsv`` /
   ``ora_gedar_down.tsv``.
...
``gsea_gedar_results.tsv``
   Optional GEDAR preranked GSEA results with the same column meanings as the
   correlation GSEA table.

``ora_gedar_up.tsv`` and ``ora_gedar_down.tsv``
   Optional GEDAR ORA results for the selected up- and down-weighted genes,
   using the same schema as the correlation ORA tables.
...
``ensemble_corr_results.tsv``
   Correlation ensemble-enrichment results with:
...
``ensemble_pls<n>_results.tsv``
   Ensemble-enrichment results for one PLS component with the same column
   meanings as the correlation ensemble table.
